...
class IsCashierUser(BasePermission):
    def has_permission(self, request, view):
        permission_data = get_permission_data(request, 'cashierpermission')
        return permission_data.get("permission")


class IsBaristaUser(BasePermission):
    def has_permission(self, request, view):
        permission_data = get_permission_data(request, 'baristapermission')
        return permission_data.get("permission")


class IsManagerUser(BasePermission):
    def has_permission(self, request, view):
        permission_data = get_permission_data(request, 'menagerpermission')
        return permission_data.get("permission")


class IsOwnerUser(BasePermission):
    def has_permission(self, request, view):
        permission_data = get_permission_data(request, 'ownerpermission')
        return permission_data.get("permission")
...
class MenuViewPermission(BasePermission):
    ...
    def fetch_permission(self, request):
        ...
        if self.permission_dict is None:
            permission_data = get_permission_data(request, 'menupermision')
    ...
    def has_permission(self, request, view):
        has_permission_dict = self.fetch_permission(request).get('has_permission')
        return has_permission_dict.get(view.action)

    def has_object_permission(self, request, view, obj):
        has_object_permission_dict = self.fetch_permission(request).get('has_object_permission')
